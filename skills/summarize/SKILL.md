...
检查 `.devpipe/prd.md` 和 `.devpipe/coding-plan.md` 是否存在（用于生成文档）：
...
| 来源 | 提取内容 | 必需 |
|------|----------|------|
| `.devpipe/context.json` | 开发类型、描述、卡片号、远程分支、本地分支、开发开始日期 | 是 |
| `.devpipe/prd.md` | 原始需求内容、需求分析、实现方案 | 是 |
| `git diff --stat HEAD~1 HEAD`（或对应 commit 范围） | 代码变更统计（辅助理解改动范围） | 否 |
| `git log --oneline -20` | 提交历史（辅助理解变更范围） | 否 |
| `.devpipe/coding-plan.md` | 技术方案、模块设计 | 是 |
| `.devpipe/task-progress.md` | 开发过程中的问题记录、解决方案（见"问题与解决方案记录"章节） | 是 |
| `.devpipe/review-status.md` | 评审问题和修复情况（可选，仅完整模式生成） | 否 |
...
文档写入 `.devpipe/summary.md`（固定路径），供 Dashboard 在"总结"和"已完成"阶段展示。

`.devpipe/` 已通过 bind mount 持久化到 `.devpipe/docs/<name>/` 目录，环境清理后文件自动保留，无需额外归档。

**保存步骤：**

1. 使用 Write 工具将文档写入 `.devpipe/summary.md`
...
2. **保留** `.devpipe/summary.md`（不删除，供 Dashboard 在"总结"和"已完成"阶段持续展示）

> **设计理由**：所有产出文件（`context.json`、`prd.md`、`coding-plan.md`、`task-progress.md`、`review-status.md`、`summary.md`）均保留用于归档。`.devpipe/` 已通过 bind mount 持久化到 `.devpipe/docs/` 目录，环境清理不影响这些文件。
...
迭代文档：
  - .devpipe/summary.md（Dashboard 展示用，已持久化到 .devpipe/docs/ 目录）
内容：基本信息 / 原始需求 / 需求分析过程 / 实现方案 / 问题与解决方案 / 反思与复盘
状态：所有产出文件已保留（context.json / prd.md / coding-plan.md / task-progress.md / review-status.md / summary.md）
工作流状态：done
...
| 状态 | 恢复行为 |
|------|----------|
| stage 为 `summarize` + `.devpipe/summary.md` 已存在 | 提示文档已生成，询问是否重新生成或直接清理 |
| stage 为 `summarize` + `.devpipe/summary.md` 不存在 | 从步骤 2 继续 |
...
| 依赖 | 用途 | 使用位置 |
|------|------|----------|
| `.devpipe/context.json` | 元数据（分支、卡片号等） | 步骤 2 收集资料 |
| `.devpipe/prd.md` | 需求和设计信息 | 步骤 2 收集资料 |
| `.devpipe/coding-plan.md` | 技术方案、模块设计 | 步骤 2 收集资料 |
| `.devpipe/task-progress.md` | 问题与解决方案记录 | 步骤 2 收集资料 |
| `.devpipe/review-status.md` | 评审问题和修复情况（可选） | 步骤 2 收集资料 |
